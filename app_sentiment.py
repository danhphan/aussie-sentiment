...
app.layout = html.Div(
    [   html.Div(className='container-fluid', children=[html.H2('Live Twitter Sentiment of Transport in Australia', style={'color':"#CECECE"}),
...
def update_recent_tweets(sentiment_term, intervals):
    if sentiment_term:
...
def update_pie_chart(sentiment_term, intervals):
...
def update_graph_scatter(sentiment_term, intervals):
    try:
        if sentiment_term:
...
def update_hist_graph_scatter(sentiment_term, intervals):
    try:
        if sentiment_term:
...
def update_related_terms(sentiment_term, intervals):
    try:
...
def update_recent_trending(sentiment_term, intervals):
    try:
        query = """
                SELECT
                        value
